...
export default function Modal({ item, theme, onClose }) {
  ...
  return (
    // KEY FIX: position fixed + top/left/right/bottom 0 + flexbox centering
    // This guarantees the overlay covers the full viewport regardless of scroll position
    <div
      onClick={onClose}
      style={{
        position: 'fixed',
        ...
        // Prevent the overlay itself from scrolling — the inner box scrolls instead
        overflow: 'hidden',
      }}
    >
      <div
        ...
        style={{
          ...
          // Max height relative to viewport so it never goes off screen
          maxHeight: 'calc(100vh - 40px)',
          overflowY: 'auto',
          boxShadow: '0 40px 100px rgba(0,0,0,0.28)',
          animation: 'modalPop 0.28s cubic-bezier(0.34,1.56,0.64,1)',
          // Ensure it doesn't stretch taller than content
          position: 'relative',
        }}
      >
        {/* Gradient stripe */}
                ...
                {isGallery && (
                  <>
                    <button
                      ...
                      style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 40, height: 40, borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: 'none', color: '#fff', fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    >‹</button>
                    <button
                      onClick={() => setImgIdx(i => (i + 1) % images.length)}
                      style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 40, height: 40, borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: 'none', color: '#fff', fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    >›</button>
                ...
                <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4 }}>
                  {images.map((img, i) => (
                    <img key={i} src={img} alt={`thumb ${i + 1}`} onClick={() => setImgIdx(i)}
                      style={{ width: 66, height: 50, objectFit: 'cover', borderRadius: 9, flexShrink: 0, cursor: 'pointer', border: `2.5px solid ${i === imgIdx ? accent : '#E5E7EB'}`, opacity: i === imgIdx ? 1 : 0.55, transition: 'all 0.15s' }}
                    />
                  ))}
                </div>
              )}
