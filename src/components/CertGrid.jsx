...
// FIX: import useEffect at top — never use require() inside a component
const isPdf = url => url && url.toLowerCase().includes('.pdf')

function CertModal({ cert, theme, onClose }) {
  // FIX: useEffect imported at top of file, not via require()
  useEffect(() => {
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{padding:'0 28px 28px'}}>
          <img src={cert.image_url} alt={cert.title} style={{
            width:'100%',borderRadius:14,border:'1px solid #EDEDED',display:'block',
          }}/>
          {cert.credential_url && (
...
export default function CertGrid({ certs, theme }) {
  ...
  return (
    <>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(272px,1fr))',gap:13}}>
        {certs.map(c => {
